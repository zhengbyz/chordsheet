...
浏览器打开 http://127.0.0.1:8000，把音频拖进去即可。界面包含：

- **和弦音轨** —— 横向时间轴，每小节一块。**颜色深浅代表纯度**，虚线框标出调外和弦，
  点任意小节跳转播放
- **钢琴键盘** —— 跟随播放高亮当前和弦的三个音，根音用不同颜色
- **可信度面板** —— 调号 / 大小调分开显示，速度交叉验证、纯度、调内占比一目了然
- **实时进度** —— 分析要 1-2 分钟，逐阶段显示「检测节拍」「识别和弦」而不是干转圈

**音频只在本机处理**：madmom 是编译扩展只能在服务端跑，所以形态是「本地服务 +
浏览器界面」，不需要联网，文件不会上传到任何地方。默认只监听 `127.0.0.1`。
